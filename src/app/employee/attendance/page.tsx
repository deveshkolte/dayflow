...
type DisplayStatus = AttendanceStatus | "Working" | "Not started" | "Not checked in yet";
type AttendanceRow = { dateKey: string; dateLabel: string; checkIn: string | null; checkOut: string | null };

const REQUIRED_WORK_MINUTES = 8 * 60;
const WORKDAY_END = { hour: 17, minute: 30 };
const approvedLeaveDates = new Set(["2026-08-19"]);
...
export default function EmployeeAttendancePage() {
  ...
  const foundTodayIndex = rows.findIndex((row) => row.dateKey === todayKey);
  const todayIndex = foundTodayIndex >= 0 ? foundTodayIndex : rows.length - 1;
  const today = rows[todayIndex];
  ...
  const summary = useMemo(() => {
    ...
    rows.forEach((row) => {
      const status = calculateStatus(row, now);
      if (status in counts) counts[status as AttendanceStatus] += 1;
    });
    return counts;
  }, [now, rows]);

  const todayStatus = calculateStatus(today, now);
  const isOnLeave = todayStatus === "Leave";
  const isComplete = todayStatus === "Present" || todayStatus === "Half-day";
  const isCheckedIn = todayStatus === "Working";
  const ActionIcon = isComplete ? CheckCircle2 : isCheckedIn ? Timer : Clock3;

  function currentTime() { return `${String(new Date().getHours()).padStart(2, "0")}:${String(new Date().getMinutes()).padStart(2, "0")}`; }

  function checkIn() {
    if (isOnLeave) {
      setError("You are on approved leave today, so check-in is unavailable.");
      return;
    }
    if (today.checkIn) {
    ...
    setRows((current) => current.map((row, index) => index === todayIndex ? { ...row, checkIn: currentTime() } : row));
    setFeedback("You're checked in! Have a great day.");
  }

  function checkOut() {
    if (isOnLeave) {
      setError("You are on approved leave today, so check-out is unavailable.");
      return;
    }
    if (!today.checkIn) {
    ...
    setRows((current) => current.map((row, index) => index === todayIndex ? { ...row, checkOut: currentTime() } : row));
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 grid gap-6 xl:grid-cols-[1.2fr_1fr]">
          <div className={`relative overflow-hidden rounded-xl border p-6 text-white shadow-sm transition ${isComplete ? "border-[#6d7f53] bg-[#454f2d]" : "border-[#394032] bg-[#394032]"}`}>
            ...
            <div className="relative flex flex-col gap-8 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#e2c58e]">Today&apos;s attendance</p><p className="mt-4 text-4xl font-bold tracking-tight">{now.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</p><p className="mt-2 text-sm text-white/65">{isComplete ? "You're all done for today!" : isCheckedIn ? "Wrapping up for today?" : todayStatus === "Leave" ? "On approved leave today." : todayStatus === "Absent" ? "The working day ended without a check-in." : "Ready to start your day?"}</p></div><div className={`flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-4 ${isComplete || isCheckedIn ? "border-[#e2c58e] bg-[#e2c58e]/10" : "border-white/15 bg-white/5"}`}><ActionIcon className="h-9 w-9 text-[#e2c58e]" /></div></div>
            <div className="relative mt-8 grid gap-3 sm:grid-cols-2"><button onClick={checkIn} disabled={Boolean(today.checkIn) || isOnLeave || todayStatus === "Absent"} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg bg-[#e2c58e] px-4 py-3 text-sm font-bold text-[#394032] transition hover:bg-[#f0d9a9] disabled:cursor-not-allowed disabled:opacity-45"><LogIn className="h-5 w-5" /> {isOnLeave ? "On leave" : todayStatus === "Absent" ? "Day ended" : today.checkIn ? "Checked in" : "Check in"}</button><button onClick={checkOut} disabled={!today.checkIn || Boolean(today.checkOut) || isOnLeave} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-lg border border-white/25 px-4 py-3 text-sm font-bold text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-40"><LogOut className="h-5 w-5" /> {isComplete ? "Day completed" : isOnLeave ? "On leave" : "Check out"}</button></div>
            <div className="relative mt-6 grid grid-cols-2 gap-4 border-t border-white/15 pt-5 text-sm"><div><p className="text-white/55">Check-in time</p><p className="mt-1 font-semibold">{formatTime(today.checkIn)}</p></div><div><p className="text-white/55">Check-out time</p><p className="mt-1 font-semibold">{formatTime(today.checkOut)}</p></div></div>
        ...
        <section className="mt-6 rounded-xl border border-[#ded9cf] bg-white p-6 shadow-sm"><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center"><div><p className="text-sm font-semibold uppercase tracking-[0.16em] text-[#797f3e]">History</p><h2 className="mt-2 text-xl font-bold text-[#394032]">Attendance records</h2></div><div className="flex rounded-lg border border-[#ded9cf] p-1">{(["Daily", "Weekly"] as const).map((option) => <button key={option} onClick={() => setView(option)} className={`rounded-md px-4 py-2 text-sm font-semibold transition ${view === option ? "bg-[#454f2d] text-white" : "text-[#6d6a61] hover:bg-[#eef0e4]"}`}>{option}</button>)}</div></div>
          {view === "Daily" ? <div className="mt-6 grid gap-4 sm:grid-cols-3"><Detail label="Check-in" value={formatTime(today.checkIn)} /><Detail label="Check-out" value={formatTime(today.checkOut)} /><Detail label="Working hours" value={calculateHours(today.checkIn, today.checkOut, now, today.dateKey === todayKey)} /><div className="sm:col-span-3"><div className="flex items-center justify-between rounded-lg bg-[#f7f6f1] p-4"><span className="text-sm font-semibold text-[#6d6a61]">Today&apos;s status</span><Status status={todayStatus} /></div></div></div> : <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-b border-[#ded9cf] text-xs uppercase tracking-wide text-[#6d6a61]"><tr><th className="pb-3">Date</th><th className="pb-3">Check in</th><th className="pb-3">Check out</th><th className="pb-3">Working hours</th><th className="pb-3">Status</th></tr></thead><tbody>{rows.map((row) => <tr key={row.dateKey} className="border-b border-[#ded9cf] last:border-0"><td className="py-4 font-semibold">{row.dateLabel}</td><td className="py-4">{formatTime(row.checkIn)}</td><td className="py-4">{formatTime(row.checkOut)}</td><td className="py-4">{calculateHours(row.checkIn, row.checkOut, now, row.dateKey === todayKey)}</td><td className="py-4"><Status status={calculateStatus(row, now)} /></td></tr>)}</tbody></table></div>}
        </section>
      </div>
    </main>
  );
...
function parseTime(value: string | null, dateKey = "2026-08-22") { if (!value) return null; const [hours, minutes] = value.split(":").map(Number); const date = new Date(`${dateKey}T00:00:00`); date.setHours(hours, minutes, 0, 0); return date; }
function workedMinutes(checkIn: string | null, checkOut: string | null, now?: Date, dateKey?: string) { const start = parseTime(checkIn, dateKey); const end = parseTime(checkOut, dateKey) ?? (now && checkIn ? now : null); return start && end ? Math.max(0, Math.round((end.getTime() - start.getTime()) / 60000)) : 0; }
function calculateHours(checkIn: string | null, checkOut: string | null, now?: Date, isToday = false) { const minutes = workedMinutes(checkIn, checkOut, isToday ? now : undefined); return minutes ? `${Math.floor(minutes / 60)}h ${minutes % 60}m` : "-"; }
function calculateStatus(row: AttendanceRow, now: Date): DisplayStatus { if (approvedLeaveDates.has(row.dateKey)) return "Leave"; if (row.checkIn && row.checkOut) return workedMinutes(row.checkIn, row.checkOut, undefined, row.dateKey) >= REQUIRED_WORK_MINUTES ? "Present" : "Half-day"; if (row.checkIn) return "Working"; const todayKey = toDateKey(now); if (row.dateKey > todayKey) return "Not started"; if (row.dateKey < todayKey || (row.dateKey === todayKey && (now.getHours() > WORKDAY_END.hour || (now.getHours() === WORKDAY_END.hour && now.getMinutes() >= WORKDAY_END.minute)))) return "Absent"; return "Not checked in yet"; }
...
function formatTime(value: string | null) { if (!value) return "Not recorded"; const [hours, minutes] = value.split(":").map(Number); const date = new Date(); date.setHours(hours, minutes, 0, 0); return date.toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }); }
function Status({ status }: { status: DisplayStatus }) { return <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ${status === "Present" ? "bg-[#eef0e4] text-[#454f2d]" : status === "Leave" ? "bg-[#f1eee7] text-[#9f7e4a]" : status === "Half-day" ? "bg-[#f8f0df] text-[#9f7e4a]" : status === "Working" ? "bg-[#e7eee2] text-[#454f2d]" : status === "Not checked in yet" || status === "Not started" ? "bg-[#f1eee7] text-[#6d6a61]" : "bg-[#f7e9e3] text-[#8b5b48]"}`}>{status}</span>; }
